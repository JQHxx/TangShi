...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        imgView.backgroundColor = kSystemColor;
        ...
        _cookStepLable = [[UILabel alloc] initWithFrame:CGRectMake(imgView.right + 5, imgView.top, kScreenWidth - 20 - 30 * 2 - 5, 20)];
        _cookStepLable.font=kFontWithSize(14);
        _cookStepLable.textColor=[UIColor colorWithHexString:@"666666"];
        _cookStepLable.numberOfLines = 0;
        [self.contentView addSubview:_cookStepLable];
        
        CGSize sizeImg = CGSizeMake(130, 100);
        _stepImg = [[UIImageView alloc] initWithFrame:CGRectMake(20,_cookStepLable.bottom+30, kScreenWidth - 20 * 2,((kScreenWidth - 20 * 2)/sizeImg.width) * sizeImg.height )];
        [self.contentView addSubview:_stepImg];
...
- (void)cellInitWithData:(TJYCookSetpModel *)model{
    CGSize textHeight = [model.brief boundingRectWithSize:CGSizeMake( kScreenWidth - 20 - 30 * 2 - 5, 1000) withTextFont:kFontWithSize(14)];
...
+(CGFloat)returnRowHeightForObject:(id)object isScrollDown:(BOOL)isFlag
{
    ...
    CGFloat height = 0;
    CGSize textHeight = [stepModel.brief boundingRectWithSize:CGSizeMake(kScreenWidth - 20 - 30 * 2 - 5, 1000) withTextFont:kFontWithSize(14)];
    height = height + 30;
