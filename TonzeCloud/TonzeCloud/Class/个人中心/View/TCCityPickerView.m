...
- (void)initBaseData{
    NSString *txtPath = [[NSBundle mainBundle] pathForResource:@"address" ofType:@"txt"];
    NSString *path = [[NSString  alloc] initWithContentsOfFile:txtPath encoding:NSUTF8StringEncoding error:nil];
    NSData *JSONData = [path dataUsingEncoding:NSUTF8StringEncoding];
    NSDictionary *addressDict = [NSJSONSerialization JSONObjectWithData:JSONData options:NSJSONReadingMutableContainers error:nil];
    
    NSDictionary *resultDict = [addressDict objectForKey:@"Result"];
    NSDictionary *provinceItemsDict = [resultDict objectForKey:@"ProvinceItems"];
    self.allCityInfo = [provinceItemsDict objectForKey:@"Province"];
    
    NSMutableArray *tempArr = [NSMutableArray array];
    for (NSInteger i = 0; i < self.allCityInfo.count; i++) {
        NSDictionary *proviceDict = self.allCityInfo[i];
        [tempArr addObject:[proviceDict objectForKey:@"Name"]];
    }
    self.provinceArr  = [tempArr copy];
    
    self.cityArr = [self getNameforProvince:0];
    self.townArr = [self.cityArr[0] objectForKey:@"Area"];
    
    self.province = self.provinceArr[0];
    self.city = [self.cityArr[0] objectForKey:@"Name"];
    self.town =  [self.townArr[0] objectForKey:@"Name"];
}
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    if (component == 0) {
        ...
        label.text =  [self.cityArr[row] objectForKey:@"Name"];
    }else if (component == 2){
        label.text =  [self.townArr[row] objectForKey:@"Name"];
    }
    return label;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    if (component == 0) {//选择省
        ...
        self.townArr = [self.cityArr[row] objectForKey:@"Area"];
        [self.cityPickerView reloadComponent:2];
        [self.cityPickerView selectRow:0 inComponent:2 animated:YES];
        self.city = [self.cityArr[row] objectForKey:@"Name"];
        self.town = [self.townArr[0] objectForKey:@"Name"];
    }else if (component == 2){
        self.town = [self.townArr[row] objectForKey:@"Name"];
    }
}
