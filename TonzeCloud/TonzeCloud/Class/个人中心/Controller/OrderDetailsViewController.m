...
- (void)requestOrderDetailsData{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KShopOrderDetail body:body success:^(id json) {
        ...
        if ([weakSelf.orderInfoModel.order_status isEqualToString:@"unpayed"]) {
            ...
            [QLCoreTextManager setAttributedValue:attStr text:@"应付金额:" font:kFontWithSize(15 * kScreenWidth/375) color:UIColorHex(0x999999)];
            ...
            weakSelf.orderPriceLab.hidden = NO;
            weakSelf.orderPriceLab.font = kFontWithSize(14);
            ...
            NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc]initWithString:totalStr];
            [QLCoreTextManager setAttributedValue:attStr text:@"售后请联系客服：" font:kFontWithSize(12) color:UIColorFromRGB(0x999999)];
            weakSelf.orderPriceLab.attributedText = attStr;
            [weakSelf.rightBtn setTitle:@"再次购买" forState:UIControlStateNormal];
...
-(void)timerFirMethod:(NSTimer *)theTimer{
    ...
    NSTimeInterval timeInterval = [[TCHelper sharedTCHelper]getOrderCountdownWithCreationTime:_orderInfoModel.created];
    ...
    if (hours<=0&&minutes<=0&&seconds<=0) {
        ...
        [TCHelper sharedTCHelper].isOrderListReload = YES;
        kSelfWeak; // 延迟30刷新订单详情页面
...
- (void)toolbarAction:(UIButton *)sender{
    ...
    if (sender.tag == 1000) {
        if ([_orderInfoModel.order_status isEqualToString:@"unpayed"]) {
            // 待付款 - 取消订单
            [[TCHelper sharedTCHelper]loginClick:@"003-20-02"];
            [MobClick event:@"104_003047"];
            ...
            alertView.resultIndex = ^(NSInteger index){
                ...
                [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KStatusUpdate body:body success:^(id json) {
            ...
            // 待收货 - 查看物流
            [[TCHelper sharedTCHelper]loginClick:@"003-20-04"];
            [MobClick event:@"104_003049"];
            ...
            // 已取消 - 删除订单
            [[TCHelper sharedTCHelper]loginClick:@"003-20-07"];
            [MobClick event:@"104_003052"];
            ...
            alertView.resultIndex = ^(NSInteger index){
                ...
                [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:kDeleaterOrder body:body success:^(id json) {
        ...
        if ([_orderInfoModel.order_status isEqualToString:@"unpayed"]) {
            // 等待付款 - 立即付款
            [[TCHelper sharedTCHelper]loginClick:@"003-20-03"];
            [MobClick event:@"104_003048"];
            ...
            // 待收货 - 确认收货
            [[TCHelper sharedTCHelper]loginClick:@"003-20-05"];
            [MobClick event:@"104_003050"];
            
            QLAlertView *alertView = [[QLAlertView alloc]initWithTitle:@"" message:@"是否确认收货？" sureBtn:@"取消" cancleBtn:@"确定"];
            kSelfWeak;
            alertView.resultIndex = ^(NSInteger index){
                NSString *body = [NSString stringWithFormat:@"member_id=%@&order_id=%@",[NSUserDefaultsInfos getValueforKey:USER_ID],_orderInfoModel.order_id];
                [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:kOrderReceive body:body success:^(id json) {
                    [TCHelper sharedTCHelper].isOrderListReload = YES;
            ...
            // 交易关闭 - 再次购买
            if ([_orderInfoModel.order_status isEqualToString:@"finish"]) {
                [[TCHelper sharedTCHelper]loginClick:@"003-20-06"];
                [MobClick event:@"104_003051"];
            }else{
                // 重新购买数据埋点
                [[TCHelper sharedTCHelper]loginClick:@"003-20-08"];
                [MobClick event:@"104_003053"];
            }
            
            NSString *memberId = [NSUserDefaultsInfos getValueforKey:USER_ID];
            NSString *body = [NSString stringWithFormat:@"member_id=%@&order_id=%@",memberId,_orderId];
            kSelfWeak;
            [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KBuyAgain body:body success:^(id json) {
...
- (CGFloat )calculateHeightWithStr:(NSString *)str{
    ...
    CGSize addStrSize = [str boundingRectWithSize:CGSizeMake(kScreenWidth - 52, 100) withTextFont:kFontWithSize(13)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    titleLab.font = kFontWithSize(12);
    ...
    switch (section) {
        case 0:
        {
            ...
            sectionHeaderView.backgroundColor = UIColorFromRGB(0x43DF90);
            ...
            if ([_orderInfoModel.order_status isEqualToString:@"unpayed"]) {
                ...
                iconImg.image = [UIImage imageNamed:@"pd_ic_send"];
                 ...
                 titleLab.text = @"交易关闭";
                 iconImg.image = [UIImage imageNamed:@"pd_ic_closed"];
            }
        }break;
        case 1:
        {
            ...
            if ([_orderInfoModel.order_status isEqualToString:@"unpayed"] || [_orderInfoModel.order_status isEqualToString:@"dead"]) {
                ...
                if (indexPath.row == 0) {
                    ...
                    if (!kIsEmptyString(_orderInfoModel.buyer_memo)) {
                        subscriberInfoCell.iconImgStr = @"pd_ic_msg";
                        subscriberInfoCell.titleStr = @"买家留言";
                        subscriberInfoCell.subStr = kIsEmptyString(_orderInfoModel.buyer_memo)|| [_orderInfoModel.buyer_memo isEqualToString:@"(null)"] ? @"暂无留言" : _orderInfoModel.buyer_memo;
                    }
                }
            }
            ...
            if ([_orderInfoModel.order_status isEqualToString:@"unpayed"] || [_orderInfoModel.order_status isEqualToString:@"dead"]) {
                if (indexPath.row == 0 ) {
                    ...
                    orderDetailsCell.titleLab.text = @"提交时间：";
                     orderDetailsCell.contentLab.text = kIsEmptyString(_orderInfoModel.created) ? @"" :[[TCHelper sharedTCHelper]timeWithTimeIntervalString:_orderInfoModel.created format:@"yyyy-MM-dd HH:mm"];
                ...
                if (indexPath.row == 0) {
                    ...
                    orderDetailsCell.contentLab.text = kIsEmptyString(_orderInfoModel.lastmodify) ? @"" : [[TCHelper sharedTCHelper]timeWithTimeIntervalString:_orderInfoModel.lastmodify format:@"yyyy-MM-dd HH:mm"];
                }
            }
...
- (void)didSelectDuplicateOrder{
    // --  复制订单编号
    [[TCHelper sharedTCHelper]loginClick:@"003-20-01"];
    [MobClick event:@"104_003046"];
...
- (UITableView *)orderDetailsTab{
    if (!_orderDetailsTab) {
        _orderDetailsTab = [[UITableView alloc]initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, kRootViewHeight - 44) style:UITableViewStyleGrouped];
...
- (UIView *)toolbarView{
    if (!_toolbarView) {
        ...
        CALayer *lens = [[CALayer alloc]init];
        lens.frame = CGRectMake(0, 0, kScreenWidth, 0.5);
        lens.backgroundColor = UIColorFromRGB(0xe5e5e5).CGColor;
        [_toolbarView.layer addSublayer:lens];
        
        _orderPriceLab = [[UILabel alloc]initWithFrame:CGRectMake(15, (_toolbarView.height - 20)/2, kScreenWidth - 80 - 30, 20)];
        _orderPriceLab.textColor = UIColorFromRGB(0xf33f00);
        _orderPriceLab.font = kFontWithSize(18 * kScreenWidth/375);
        ...
        _leftBtn.titleLabel.font = kFontWithSize(13);
        [_leftBtn setTitleColor:UIColorFromRGB(0x626262) forState:UIControlStateNormal];
        ...
        _leftBtn.tag = 1000;
        _leftBtn.hidden = YES;
        _leftBtn.layer.borderColor = UIColorFromRGB(0x626262).CGColor;
        ...
        _rightBtn.titleLabel.font = kFontWithSize(13);
        [_rightBtn setTitleColor:UIColorFromRGB(0x626262) forState:UIControlStateNormal];
...
- (UILabel *)countdownLab{
    if (!_countdownLab) {
        ...
        _countdownLab.font = kFontWithSize(15);
        _countdownLab.textAlignment = NSTextAlignmentRight;
        _countdownLab.textColor = UIColorFromRGB(0xffffff);
    }
