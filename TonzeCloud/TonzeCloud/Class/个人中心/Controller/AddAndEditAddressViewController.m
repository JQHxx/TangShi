...
-(void)requestAddAndEditAddressData{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KGetAllRegions body:body success:^(id json) {
...
- (void)rightButtonAction{
    ...
    alertView.resultIndex = ^(NSInteger index){
        ...
        [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KDeleateAddress body:body success:^(id json) {
            ...
            [TCHelper sharedTCHelper].isAddressManagerReload = YES;
...
- (void)saveAddress{
    ...
    BOOL hasEmoji = [[TCHelper sharedTCHelper] strIsContainEmojiWithStr:_addressTF.text];
    // 保存地址
    if (kIsEmptyString(_nameTF.text)) {
        ...
        return;
    }else if ([[TCHelper sharedTCHelper] strIsContainEmojiWithStr:_nameTF.text]){
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    ...
    if (indexPath.row == 2) {
        ...
        if (kIsArray(_areaArray) && _areaArray.count > 0) {
            ...
            self.cityPickerView.pickerStyle = PickerStyle_city;
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    defaultTipLab.font = kFontWithSize(15);
    defaultTipLab.textColor = UIColorFromRGB(0x313131);
...
- (UITableView *)addressInfoTab{
    if (!_addressInfoTab) {
        _addressInfoTab = [[UITableView alloc]initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, kRootViewHeight) style:UITableViewStylePlain];
