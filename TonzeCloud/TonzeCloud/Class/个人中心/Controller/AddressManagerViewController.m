...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if ([TCHelper sharedTCHelper].isAddressManagerReload) {
        [self loadNewAddressData];
        [TCHelper sharedTCHelper].isAddressManagerReload = NO;
        [TCHelper sharedTCHelper].isAddressSelectedReload=YES;
    }
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)requestAddressManagerData{
    ...
    NSString *memberIdStr = [NSUserDefaultsInfos getValueforKey:USER_ID];
...
- (void)didSelectDeleteAddressInCell:(UITableViewCell *)cell{
    [[TCHelper sharedTCHelper]loginClick:@"003-18-04"];
    [MobClick event:@"104_002063"];
    ...
    alertView.resultIndex = ^(NSInteger index){
        if (index == 1) {
            ...
            [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KDeleateAddress body:body success:^(id json) {
                [TCHelper sharedTCHelper].isAddressSelectedReload=YES;
...
- (void)didSelectEditAddressInCell:(UITableViewCell *)cell{
    [[TCHelper sharedTCHelper]loginClick:@"003-18-03"];
    [MobClick event:@"104_002062"];
    NSIndexPath *indexPath = [self.addressManagerTb indexPathForCell:cell];
...
- (void)didSelectDefaultAddressInCell:(UITableViewCell *)cell{
    [[TCHelper sharedTCHelper]loginClick:@"003-18-02"];
    [MobClick event:@"104_002061"];
    NSIndexPath *indexPath = [self.addressManagerTb indexPathForCell:cell];
    ShippingAddressModel *addressModel = self.addressListArray[indexPath.row];
    if ([addressModel.is_default isEqualToString:@"false"]) {
        ...
        kSelfWeak;
        [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KSaveAddress body:body success:^(id json) {
...
// 添加新地址
- (void)addNewAddress{
    [[TCHelper sharedTCHelper]loginClick:@"003-18-01"];
    [MobClick event:@"104_002060"];
...
- (UITableView *)addressManagerTb{
    if (!_addressManagerTb) {
        _addressManagerTb = [[UITableView alloc]initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, kRootViewHeight - 40)];
...
- (UIButton *)addAddressBtn{
    if (!_addAddressBtn) {
        ...
        [_addAddressBtn setTitleColor:UIColorFromRGB(0xffffff) forState:UIControlStateNormal];
        _addAddressBtn.titleLabel.font = kFontWithSize(18);
