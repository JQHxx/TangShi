...
@interface OrderViewController ()<TCMenuViewDelegate,UITableViewDelegate,UITableViewDataSource>{
    NSInteger   _page;
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if ([TCHelper sharedTCHelper].isOrderListReload) {
        [TCHelper sharedTCHelper].isOrderListReload = NO;
...
- (void)requestQrderDataWithIndexStatus:(NSInteger)indexStatus{
    ...
    NSString *body = [NSString stringWithFormat:@"member_id=%@&page_num=%ld&page_size=15&order_status=%@&platform=ts",memberId,_page,orderStatuStr];
    
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:KShopOrderList body:body success:^(id json) {
...
- (void)editShop:(UIButton *)sender{
    ...
    if ([orderModer.order_status isEqualToString:@"unpayed"]) {
        ...
        if (sender.tag  < 1999) {// 取消订单
            [MobClick event:@"104_002053"];
            [[TCHelper sharedTCHelper]loginClick:@"003-17-03"];
            ...
            [[TCHelper sharedTCHelper]loginClick:@"003-17-04"];
            [MobClick event:@"104_002054"];
        ...
        // 待发货 -- 查看物流
        [[TCHelper sharedTCHelper]loginClick:@"003-17-05"];
        [MobClick event:@"104_002055"];
        ...
        if (sender.tag < 1999) {// 查看物流
            ...
            [[TCHelper sharedTCHelper]loginClick:@"003-17-06"];
            [MobClick event:@"104_002056"];
            QLAlertView *alertView = [[QLAlertView alloc]initWithTitle:@"" message:@"是否确认收货？" sureBtn:@"取消" cancleBtn:@"确定"];
            kSelfWeak;
            alertView.resultIndex = ^(NSInteger index){
                NSString *body = [NSString stringWithFormat:@"member_id=%@&order_id=%@",[NSUserDefaultsInfos getValueforKey:USER_ID],orderModer.order_id];
        ...
        [[TCHelper sharedTCHelper]loginClick:@"003-17-07"];
        [MobClick event:@"104_002057"];
        NSString *memberId = [NSUserDefaultsInfos getValueforKey:USER_ID];
        ...
        if (sender.tag < 1999) {// 删除订单
            [[TCHelper sharedTCHelper]loginClick:@"003-17-08"];
            [MobClick event:@"104_002058"];
            ...
            [[TCHelper sharedTCHelper]loginClick:@"003-17-09"];
            [MobClick event:@"104_002059"];
...
- (void)menuView:(TCMenuView *)menuView actionWithIndex:(NSInteger)index{
    ...
    [[TCHelper sharedTCHelper]loginClick:@"003-17-01"];
    [MobClick event:@"104_002051"];
    _page = 1;
    _indexStatu = index;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    orderNoLab.font = kFontWithSize(12);
    orderNoLab.textColor = UIColorFromRGB(0x999999);
    ...
    orderTypeLab.font = kFontWithSize(12);
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    ...
    totalLab.font = kFontWithSize(12);
    totalLab.textAlignment = NSTextAlignmentRight;
    totalLab.textColor = UIColorFromRGB(0x999999);
    [sectionFooterView addSubview:totalLab];
    
    CALayer *lens = [[CALayer alloc]init];
    lens.frame =CGRectMake(15, sectionFooterView.height/2, kScreenWidth - 15, 0.5);
    lens.backgroundColor =UIColorFromRGB(0xe5e5e5).CGColor;
    [sectionFooterView.layer addSublayer:lens];
    
    UIButton  *leftBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    leftBtn.frame = CGRectMake(kScreenWidth - 177 , sectionFooterView.height/2 + 6, 76, 26);
    leftBtn.titleLabel.font = kFontWithSize(13);
    [leftBtn setTitleColor:UIColorFromRGB(0x626262) forState:UIControlStateNormal];
    ...
    leftBtn.layer.borderColor = UIColorFromRGB(0x626262).CGColor;
    ...
    rightBtn.titleLabel.font = kFontWithSize(13);
    [rightBtn setTitleColor:UIColorFromRGB(0x626262) forState:UIControlStateNormal];
    ...
    rightBtn.layer.borderColor = UIColorFromRGB(0x626262).CGColor;
    ...
    if (kIsArray(_orderListArray) && _orderListArray.count > 0) {
        ...
        if ([orderModer.order_status isEqualToString:@"unpayed"]) {
            ...
            rightBtn.backgroundColor = UIColorFromRGB(0xf33f00);
            [rightBtn setTitleColor:UIColorFromRGB(0xffffff) forState:UIControlStateNormal];
            rightBtn.layer.borderColor =UIColorFromRGB(0xf33f00).CGColor;
            totalLab.text = [NSString stringWithFormat:@"共计%@件商品 应付金额：¥%@",orderModer.itemnum,[NSString notRounding:orderModer.amount afterPoint:2]];

        }else if ([orderModer.order_status isEqualToString:@"nodelivery"]){
            // 待发货
            leftBtn.hidden = YES;
            [rightBtn setTitle:@"查看物流" forState:UIControlStateNormal];
            totalLab.text = [NSString stringWithFormat:@"共计%@件商品 已付金额：¥%@",orderModer.itemnum,[NSString notRounding:orderModer.amount afterPoint:2]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    [[TCHelper sharedTCHelper] loginClick:@"003-17-02"];
    [MobClick event:@"104_002052"];
...
- (TCMenuView *)menuView{
    if (!_menuView) {
        _menuView = [[TCMenuView alloc]initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, 49)];
...
- (UITableView *)orderTableView{
    if (!_orderTableView) {
        _orderTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, kNewNavHeight  + 49, kScreenWidth, kRootViewHeight - _menuView.height) style:UITableViewStyleGrouped];
...
- (TCBlankView *)blankView{
    if (!_blankView) {
        _blankView = [[TCBlankView alloc]initWithFrame:CGRectMake(0,90, kScreenWidth, kRootViewHeight - 90) unOrderImg:@"pd_ic_order_none" tipText:@"暂无相关的订单记录" chooseText:@"去挑选几件喜欢的商品吧"];
