...
-(instancetype)initWithHeight:(CGFloat)viewHeight btnNames:(NSArray *)btnNames btnColors:(NSArray *)btnColors{
    ...
    if (self) {
        ...
        [closeBtn setImage:[UIImage imageNamed:@"pd_ic_close"] forState:UIControlStateNormal];
...
-(void)quantityTextInConfirm{
    ...
    if (count>0) {
        if (count>self.goodsModel.store) {
            ...
            [MobClick event:@"101_003031"];
            [[TCHelper sharedTCHelper] loginClick:@"009-02-02"];
...
-(void)loadGoodsPropertyToolWithModel:(GoodsModel *)goodsModel{
    ...
    if (kIsEmptyString(imgUrl)) {
        imgView.image=[UIImage imageNamed:@"store_pd_nor"];
    }else{
        [imgView sd_setImageWithURL:[NSURL URLWithString:imgUrl] placeholderImage:[UIImage imageNamed:@"store_pd_nor"]];
    }
