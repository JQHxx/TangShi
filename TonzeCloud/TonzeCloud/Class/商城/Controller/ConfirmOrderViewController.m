...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([TCHelper sharedTCHelper].isOrderAddressReload) {
        [self requestDefaultConsigneeAddressInfo];
        [TCHelper sharedTCHelper].isOrderAddressReload=NO;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        ...
        cell.setQuantityBlock=^(NSInteger quantity){
            [[TCHelper sharedTCHelper]loginClick:@"009-04-02"];
            [MobClick event:@"101_004002"];
            if (weakSelf.isFastBuy) {
                NSString *user_id = [NSUserDefaultsInfos getValueforKey:USER_ID];
                CartGoodsModel *model=self.goodsArray[indexPath.row];
                NSString *body = [NSString stringWithFormat:@"member_id=%@&btype=is_fastbuy&goods_id=%ld&product_id=%ld&num=%ld",user_id,(long)model.goods_id,(long)model.product_id,quantity];
                ...
                [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopCartChangeNum body:body success:^(id json) {
                    ...
                    [weakSelf.listTableView reloadData];
                    [TCHelper sharedTCHelper].isCartListReload=YES;
                    [weakSelf calculateGoodsTotalPrice];
                } failure:^(NSString *errorStr) {
                    [weakSelf.view makeToast:errorStr duration:1.0 position:CSToastPositionCenter];
                }];
...
-(void)leftButtonAction{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"确定要放弃该订单吗？" preferredStyle:UIAlertControllerStyleAlert];
    ...
    [sureAction setValue:kRGBColor(109, 205, 248) forKey:@"_titleTextColor"];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string;
{
    ...
    NSString * toBeString = [textField.text stringByReplacingCharactersInRange:range withString:tem]; //得到输入框的内容
    //判断是否时我们想要限定的那个输入框
...
-(void)submitOrderAction{
    BOOL hasEmoji = [[TCHelper sharedTCHelper] strIsContainEmojiWithStr:remarkTextField.text];
    if (kIsEmptyString(myConsignee.ship_id)) {
        ...
        [MobClick event:@"101_004003"];
        [[TCHelper sharedTCHelper]loginClick:@"009-04-03"];
        kSelfWeak;
        ...
        [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopOrderCreate body:body success:^(id json) {
...
-(void)textInConsigneeInfo{
    [[TCHelper sharedTCHelper]loginClick:@"009-04-01"];
    [MobClick event:@"101_004001"];
...
-(void)requestDefaultConsigneeAddressInfo{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest]postShopMethodWithURL:kShippingAddress body:body success:^(id json) {
        NSDictionary *result=[json objectForKey:@"result"];
        if (kIsDictionary(result)&&result.count>0) {
...
-(UITableView *)listTableView{
    if (!_listTableView) {
        _listTableView=[[UITableView alloc] initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, kRootViewHeight-50) style:UITableViewStyleGrouped];
