...
- (void)viewDidLoad {
    ...
    selectedGoodsNum=0;
    memberId=[[NSUserDefaultsInfos getValueforKey:USER_ID] integerValue];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if ([TCHelper sharedTCHelper].isCartListReload) {
        [self requestShopCartData];
        [TCHelper sharedTCHelper].isCartListReload=NO;
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.selectedGoodsBlock=^(BOOL isSelected){  //选择商品
        [[TCHelper sharedTCHelper]loginClick:@"009-03-03"];
        [MobClick event:@"101_003038"];
    ...
    cell.setQuantityBlock=^(NSInteger quantity){ //修改商品数量
        [[TCHelper sharedTCHelper]loginClick:@"009-03-04"];
        [MobClick event:@"101_003039"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section==0) {
        [[TCHelper sharedTCHelper]loginClick:@"009-03-02"];
        [MobClick event:@"101_003037"];
...
-(void)goodsPropertyToolDidClickButton:(NSString *)btnName withGoodsId:(NSInteger)goods_id OldProductId:(NSInteger)oldProductId newProductId:(NSInteger)newProductId Quantity:(NSInteger)quantity{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopCartUpdate body:body success:^(id json) {
...
-(void)rightButtonAction{
    ...
    [[TCHelper sharedTCHelper]loginClick:@"009-03-01"];
    [MobClick event:@"101_003036"];
...
-(void)selectCartAllGoodsForSender:(UIButton *)sender{
    ...
    [[TCHelper sharedTCHelper]loginClick:@"009-03-05"];
    [MobClick event:@"101_003040"];
...
-(void)balanceAccountsForSender:(UIButton *)sender{
    ...
    [[TCHelper sharedTCHelper]loginClick:@"009-03-09"];
    [MobClick event:@"101_003044"];
...
-(void)addToCollectionForGoods:(UIButton *)sender{
    ...
    if (selGoodsArr.count>0) {
        [[TCHelper sharedTCHelper]loginClick:@"009-03-06"];
        [MobClick event:@"101_003041"];
        
        NSString *params=[[TCHttpRequest sharedTCHttpRequest] getValueWithParams:selGoodsArr];
        NSString *body=[NSString stringWithFormat:@"member_id=%ld&goodsProduct_id=%@&platform=ts",(long)memberId,params];
        kSelfWeak;
        [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopAddToFavorites body:body success:^(id json) {
...
-(void)deleteFromCartForGoods:(UIButton *)sender{
    ...
    if (selGoodsArr.count>0) {
        ...
        UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定"style:UIAlertActionStyleCancel handler:^(UIAlertAction *_Nonnull action) {
            [MobClick event:@"101_003042"];
            
            NSString *params=[[TCHttpRequest sharedTCHttpRequest] getValueWithParams:selGoodsArr];
            NSString *body=[NSString stringWithFormat:@"member_id=%ld&obj_ident=%@",(long)memberId,params];
            kSelfWeak;
            [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopDeleteGoods body:body success:^(id json) {
                NSMutableArray *tempSelGoodsArr=[[NSMutableArray alloc] init];
                for (CartGoodsModel *model in weakSelf.cartDataArray) {
                    if (model.select_status==1) {
                        [tempSelGoodsArr addObject:model];
...
-(void)clearUpAllValidGoods{
    ...
    UIAlertAction *sureAction = [UIAlertAction actionWithTitle:@"确定"style:UIAlertActionStyleCancel handler:^(UIAlertAction *_Nonnull action) {
        ...
        [[TCHelper sharedTCHelper]loginClick:@"009-03-08"];
        [MobClick event:@"101_003043"];
        ...
        NSString *params=[[TCHttpRequest sharedTCHttpRequest] getValueWithParams:tempValidArr];
        NSString *body=[NSString stringWithFormat:@"member_id=%ld&obj_ident=%@",(long)memberId,params];
        kSelfWeak;
        [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopDeleteGoods body:body success:^(id json) {
...
- (void)requestShopCartData{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopCartGoodsList body:body success:^(id json) {
        ...
        if ([TCHelper sharedTCHelper].isPayOrderBack) {
            [TCHelper sharedTCHelper].isPayOrderBack=NO;
...
-(void)requestForSelectGoodsOrNotWithType:(NSString *)selType objIdent:(NSString *)obj_ident isSelected:(BOOL)isSelected{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopCartSelected body:body success:^(id json) {
...
-(void)requestForChangeGoodsNumWithGoodsId:(NSInteger)goods_id productId:(NSInteger)product_id quantity:(NSInteger)quantity{
    ...
    kSelfWeak;
    [[TCHttpRequest sharedTCHttpRequest] postShopMethodWithURL:kShopCartChangeNum body:body success:^(id json) {
        for (CartGoodsModel *model in self.cartDataArray) {
...
-(UITableView *)cartTableView{
    if (!_cartTableView) {
        _cartTableView=[[UITableView alloc] initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, kRootViewHeight-50) style:UITableViewStyleGrouped];
