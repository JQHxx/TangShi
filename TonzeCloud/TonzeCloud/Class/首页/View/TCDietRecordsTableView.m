...
-(void)setDietRecordsDict:(NSDictionary *)dietRecordsDict{
    ...
    if (dietRecordsDict.count>0) {
        ...
        NSArray *periodArr=@[@"早餐",@"上午加餐",@"午餐",@"下午加餐",@"晚餐",@"晚上加餐"];
        ...
        for (NSString *periodStr in periodArr) {
            for (NSString *timeSlot in keys) {
                NSString *periodCh=[[TCHelper sharedTCHelper] getDietPeriodChNameWithPeriod:timeSlot];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *cellIdentifier=@"TCDietRecordsTableViewCell";
    TCDietRecordsTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (cell==nil) {
        cell=[[[NSBundle mainBundle] loadNibNamed:@"TCDietRecordsTableViewCell" owner:self options:nil] objectAtIndex:0];
    }
