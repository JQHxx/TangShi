...
-(instancetype)initWithFrame:(CGRect)frame type:(TCDietIntakeViewType)type{
    ...
    if (self) {
        ...
        dietCountView = [[TCDietCountView alloc] initWithFrame:CGRectMake((kScreenWidth-160)/2, 5, 160, 160)];
        ...
        [setDailyTargetBtn setImage:[UIImage imageNamed:@"ic_pub_arrow_green"] forState:UIControlStateNormal];
...
-(void)setDailyTargetAction{
    if ([_delegate respondsToSelector:@selector(dietIntakeViewDidSetDailyTargetIntake)]) {
        [_delegate dietIntakeViewDidSetDailyTargetIntake];
    }
}
