//
//  TCConsumeCaloriesViewController.m
//  TonzeCloud
//
...
@interface TCConsumeCaloriesViewController ()<UITableViewDelegate,UITableViewDataSource>{
    NSArray     *sportsArray;
}
...
@implementation TCConsumeCaloriesViewController
...
//    cell.imageView.image=[UIImage imageNamed:sportModel.icon];
...
-(UIView *)cosumeCaloriesView{
    if (_cosumeCaloriesView==nil) {
        _cosumeCaloriesView=[[UIView alloc] initWithFrame:CGRectMake(0, kNewNavHeight, kScreenWidth, 60)];
        ...
        [attributeStr addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:NSMakeRange(2, 2)];
...
-(UILabel *)sportsTitleLabel{
    if (_sportsTitleLabel==nil) {
        ...
        _sportsTitleLabel.text=@"为您推荐运动计划（任选其一）";
