...
@end
@implementation TCHistoryMenuView
...
-(void)setFoodMenusArray:(NSMutableArray *)foodMenusArray{
        ...
        if (numWidth>=kScreenWidth) {
            for (int i=0; i<foodMenusArray.count; i++) {
                ...
                btn.tag=100+i;
                [btn addTarget:self action:@selector(changeHistoryViewWithButton:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)changeHistoryViewWithButton:(UIButton *)btn{
    ...
    if (numWidth>=kScreenWidth) {
        ...
        if ([_delegate respondsToSelector:@selector(foodHistoryMenuView:actionWithIndex:)]) {
            [_delegate foodHistoryMenuView:self actionWithIndex:index];
        }
