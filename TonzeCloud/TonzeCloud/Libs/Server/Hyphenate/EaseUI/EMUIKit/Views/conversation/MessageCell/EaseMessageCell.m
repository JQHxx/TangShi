...
+ (void)initialize
{
    ...
    cell.bubbleMaxWidth = kScreenWidth-120;
                         ...
                         model:(id<IMessageModel>)model
{
    ...
    tapRecognizer.cancelsTouchesInView = NO;   //点击回调
...
+ (CGFloat)cellHeightWithModel:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:
        {
            ...
            __block CGFloat labHeight = 0;
            [textLabel setText:model.text afterInheritingLabelAttributesAndConfiguringWithBlock:^NSMutableAttributedString *(NSMutableAttributedString *mutableAttributedString) {
                labHeight = [TTTAttributedLabel sizeThatFitsAttributedString:mutableAttributedString
                                                             withConstraints:CGSizeMake(bubbleMaxWidth, MAXFLOAT)
                                                      limitedToNumberOfLines:0].height;
                return mutableAttributedString;
            }];
            height += (labHeight > 20 ? labHeight : 20) + 10;
