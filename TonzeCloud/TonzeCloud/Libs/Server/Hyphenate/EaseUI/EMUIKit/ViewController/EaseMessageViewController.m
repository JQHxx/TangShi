...
- (void)viewDidLoad {
    ...
    self.chatToolbar.backgroundColor=[UIColor colorWithHexString:@"#f7f7f7"];
    ...
    [[EaseChatBarMoreView appearance] setMoreViewBackgroundColor:[UIColor colorWithHexString:@"#f7f7f7"]];
...
- (void)_loadMessagesBefore:(NSString*)messageId
                  ...
                  timestamp:(long long)messageTimestamp
{
    __weak typeof(self) weakSelf = self;
    ...
    [self.conversation loadMessagesStartFromId:messageId count:(int)count searchDirection:EMMessageSearchDirectionUp completion:^(NSArray *aMessages, EMError *aError) {
        if (!aError && [aMessages count]) {
            ...
            [self _loadMessagesFromMyServerBefore:messageId count:count append:YES timestamp:messageTimestamp];
        }
    }];
}
...
/*!
 ...
 @param count     获取条数
 */
- (void)_loadMessagesFromMyServerBefore:(NSString*)messageId count:(NSInteger)count append:(BOOL)isAppend timestamp:(long long)timestamp{
    __weak typeof(self) weakSelf = self;
    void (^refresh)(NSArray *messages) = ^(NSArray *messages) {
        dispatch_async(_messageQueue, ^{
            //Format the message
    ...
    NSString *body=[NSString stringWithFormat:@"timestamp=%lld&group_id=%@&page_size=%ld&sort=desc",timestamp,self.serviceModel.im_groupid,(long)count];
    [[TCHttpRequest sharedTCHttpRequest] postMethodWithURL:kChatMessageList body:body success:^(id json) {
        NSArray *list=[json objectForKey:@"result"];
        if (kIsArray(list)&&list.count>0) {
            NSMutableArray *tempMessagesArr=[[NSMutableArray alloc] init];
            for (NSDictionary *dict in list) {
                TCChatModel *chat=[[TCChatModel alloc] init];
                [chat setValues:dict];
                
                EMMessageBody *messageBody=nil;
                if ([chat.type isEqualToString:@"txt"]) {
                    NSString *willSendText = [EaseConvertToCommonEmoticonsHelper convertToSystemEmoticons:chat.info];
                    messageBody = [[EMTextMessageBody alloc] initWithText:willSendText];
                    
                }else if ([chat.type isEqualToString:@"pic"]){
                    NSData *imageData=[[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:chat.info]];
                    EMImageMessageBody *imageMessageBody = [[EMImageMessageBody alloc] initWithData:imageData thumbnailData:imageData];
                    MyLog(@"imageMessage---thumbnailLocalPath:%@,thumbnailRemotePath:%@",imageMessageBody.thumbnailLocalPath,imageMessageBody.thumbnailRemotePath);
                    imageMessageBody.thumbnailDownloadStatus=EMDownloadStatusSucceed;
                    imageMessageBody.downloadStatus=EMDownloadStatusSucceed;
                    messageBody=imageMessageBody;
                }else if ([chat.type isEqualToString:@"voice"]){
                    NSData *voiceData=[[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:chat.info]];
                    EMVoiceMessageBody *voiceMessageBody = [[EMVoiceMessageBody alloc] initWithData:voiceData displayName:@"audio"];
                    voiceMessageBody.duration = [chat.seconds intValue];
                    messageBody=voiceMessageBody;
                }
                EMMessage *message = [[EMMessage alloc] initWithConversationID:self.serviceModel.im_groupid from:chat.from to:chat.to body:messageBody ext:nil];
                message.chatType=EMChatTypeGroupChat;
                message.status=EMMessageStatusSucceed;
                message.timestamp=[chat.timestamp longLongValue];
                
                NSString *currentUsername=[EMClient sharedClient].currentUsername;
                if ([chat.from isEqualToString:currentUsername]) {
                    message.direction=EMMessageDirectionSend;
                }else{
                    message.direction=EMMessageDirectionReceive;
                }
                
                if (kIsDictionary(chat.ext)&&chat.ext.count>0) {
                    message.ext=chat.ext;
                }
                
                [tempMessagesArr addObject:message];
                
            }
            [self tableViewDidFinishTriggerHeader:YES reload:YES];
            
            [[EMClient sharedClient].chatManager importMessages:tempMessagesArr completion:^(EMError *aError) {
                if (!aError) {
                    MyLog(@"导入一组消息：%ld",(long)tempMessagesArr.count);
                }else{
                    MyLog(@"aError:%@",aError.errorDescription);
                }
            }];
            refresh(tempMessagesArr);
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        timeCell.titleLabelFont=[UIFont systemFontOfSize:14];
...
- (void)messageCellSelected:(id<IMessageModel>)model
{
    ...
    switch (model.bodyType) {
        ...
        {
            NSDictionary *extDict=[[NSDictionary alloc] initWithDictionary:model.message.ext];
            if (kIsDictionary(extDict)&&extDict.count>0) {
                if (extDict[@"msg_type"]) {
                    _scrollToBottomWhenAppear = NO;
                    if ([self.delegate respondsToSelector:@selector(messageViewController:didSelectCellWithExt:)]) {
                        [self.delegate messageViewController:self didSelectCellWithExt:extDict];
                    }
                }
            }
        }
...
- (void)tableViewDidTriggerHeaderRefresh
{
    ...
    if ([self.messsagesSource count] > 0) {
        messageTimestamp = [(EMMessage *)self.messsagesSource.firstObject timestamp];
    }else {
        messageTimestamp = 0;
    }
    
    MyLog(@"messageTimestamp:%lld",messageTimestamp);
    
    [self _loadMessagesBefore:messageId count:self.messageCountOfPage append:YES timestamp:messageTimestamp];
    [self tableViewDidFinishTriggerHeader:YES reload:YES];
}
...
- (void)_sendMessage:(EMMessage *)message
{
    ...
    [[EMClient sharedClient].chatManager sendMessage:message progress:^(int progress) {
        ...
        if (!aError) {
            ...
            if (weakself.dataSource && [weakself.dataSource respondsToSelector:@selector(messageViewController:sendToMyServerForMessage:)]) {
                [weakself.dataSource messageViewController:weakself sendToMyServerForMessage:aMessage];
            }
        }else {
            MyLog(@"发送失败：%u,%@",aError.code,aError.errorDescription);
...
- (void)sendTextMessage:(NSString *)text withExt:(NSDictionary*)ext
{
    EMMessage *message = [EaseSDKHelper sendTextMessage:text
                                          ...
                                          messageType:[self _messageTypeFromConversationType]
                                           messageExt:ext];
    message.timestamp=[[TCHelper sharedTCHelper] getDateTimeTOMilliSeconds:[NSDate date]];
